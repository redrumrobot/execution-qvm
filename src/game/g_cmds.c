  ...
  else if( !Q_stricmp( arg1, "draw" ) )
  {
     ...
     !Q_stricmp( arg1, "suddendeath" ) )
   {
     ...
     else if( G_TimeTilSuddenDeath() <= g_suddenDeathVoteDelay.integer * 1000 )
     {
	  ...
	  else if( g_alienStage.integer != 2 && g_humanStage.integer != 2)
     {
      trap_SendServerCommand( ent - g_entities, va( "print \"callvote: Both teams need S3 before Sudden Death vote\n\"") );
      return;
     }
    else 
     {
